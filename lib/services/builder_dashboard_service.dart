...
class BuilderDashboardService {
 ...
 Future<BuilderDashboardModel> getDashboardStats(
  String builderId,
) async {
  try {
    // ===========================
    ...
    if (ratings.isNotEmpty) {
      final raterIds =
          ratings.map((e) => e['rater_id']).toList();
      ...
      if (customerRatings.isNotEmpty) {
        customerRating =
            customerRatings.reduce((a, b) => a + b) /
                customerRatings.length;
      }

      if (brokerRatings.isNotEmpty) {
        brokerRating =
            brokerRatings.reduce((a, b) => a + b) /
