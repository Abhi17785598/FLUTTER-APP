...
void _registerCreateListing() {
  toolRegistry.register(ToolDefinition(
    name: 'create_listing',
    description: 'Create a new property listing from collected slot-fill parameters.',
...
String _buildListingTitle(Map<String, dynamic> params) {
  ...
  final subtypeStr =
      subtype.isNotEmpty ? '${subtype[0].toUpperCase()}${subtype.substring(1)}' : 'Property';
  final dealStr = deal == 'rent' ? 'Rent' : deal == 'lease' ? 'Lease' : 'Sale';
  final locationStr =
    ...
    execute: (params, ctx) async {
      ...
      if (field.isEmpty || value == null) {
        return ToolResult.fail(
          ...
          userMessage: 'Please specify which field to update and its new value.',
        );
      }
      ...
      try {
        ...
        if (found == null) {
          return ToolResult.fail(
            'Listing not found.',
            userMessage: "I couldn't find a listing matching \"$searchQuery\".",
          );
        }
  ...
  toolRegistry.register(ToolDefinition(
    ...
    execute: (params, ctx) async {
      ...
      try {
        ...
        if (property != null) {
          ctx.navigate('/property-detail?propertyId=${property['id']}&action=schedule_visit');
        } else {
